...
export default function SignupPage() {
  ...
  // ── Redirect to landing page — do not show unstyled login ──
  if (typeof window !== "undefined") {
    const params = new URLSearchParams(window.location.search);
    if (params.get("error") === "auth_failed" || window.location.pathname === "/login") {
      window.location.href = "https://enatalk.com";
      return null;
    }
  }
  ...
  return (
    <div style={{
      ...
      minHeight: "100vh", background: "#040B1C", color: "#fff",
    }}>
      ...
      <div style={{position:"fixed",inset:0,zIndex:0,pointerEvents:"none"}}>
        <div style={{position:"absolute",inset:0,background:"radial-gradient(ellipse 80% 60% at 70% 0%,rgba(34,197,94,0.10) 0%,transparent 60%),#040B1C"}}/>
      </div>
      ...
      <div style={{position:"relative",zIndex:1,width:"100%",maxWidth:440,padding:"24px 20px"}}>
        ...
        <div style={{textAlign:"center",marginBottom:32}}>
          <a href="https://enatalk.com">
            <img src="/enatalk-logo.webp" alt="EnaTalk" style={{height:52,width:"auto",objectFit:"contain",filter:"drop-shadow(0 0 10px rgba(245,184,0,0.4))"}} onError={e=>{(e.target as HTMLImageElement).style.display="none"}}/>
          </a>
        </div>
        ...
        <div style={{background:"linear-gradient(160deg,rgba(15,25,55,0.96),rgba(10,20,40,0.98))",backdropFilter:"blur(32px)",border:"1px solid rgba(255,255,255,0.10)",borderRadius:24,padding:"32px 28px",boxShadow:"0 40px 100px rgba(0,0,0,0.6)"}}>
          ...
          {!showOtp && showEmailForm && (
            <form onSubmit={e=>{e.preventDefault();handleInitialSignup();}} style={{display:"flex",flexDirection:"column",gap:10}}>
              {[{type:"email",placeholder:"Email address",val:email,set:setEmail},{type:"password",placeholder:"Password",val:password,set:setPassword}].map(f=>(
                <input key={f.type} type={f.type} placeholder={f.placeholder} value={f.val} onChange={e=>f.set(e.target.value)} required disabled={loading} style={{width:"100%",padding:"13px 16px",background:"rgba(255,255,255,0.06)",border:"1px solid rgba(255,255,255,0.10)",borderRadius:12,color:"#fff",fontSize:14,fontFamily:"'DM Sans',sans-serif",outline:"none"}}/>
              ))}
              <select value={businessType} onChange={e=>setBusinessType(e.target.value)} required disabled={loading} style={{width:"100%",padding:"13px 16px",background:"rgba(255,255,255,0.06)",border:"1px solid rgba(255,255,255,0.10)",borderRadius:12,color:businessType?"#fff":"rgba(255,255,255,0.3)",fontSize:14,fontFamily:"'DM Sans',sans-serif",outline:"none"}}>
          ...
          {showOtp && (
            <form onSubmit={e=>{e.preventDefault();handleVerifyOtp();}} style={{display:"flex",flexDirection:"column",gap:12}}>
              <p style={{fontSize:13,color:"rgba(255,255,255,0.55)",lineHeight:1.65,textAlign:"center"}}>We sent a code to <strong style={{color:"#fff"}}>{email}</strong></p>
              <input type="text" inputMode="numeric" placeholder="Enter 6-digit code" value={otp} onChange={e=>setOtp(e.target.value)} required disabled={loading} style={{width:"100%",padding:"13px 16px",background:"rgba(255,255,255,0.06)",border:"1px solid rgba(255,255,255,0.10)",borderRadius:12,color:"#fff",fontSize:14,fontFamily:"'DM Sans',sans-serif",outline:"none",textAlign:"center",letterSpacing:4,fontSize:20}}/>
