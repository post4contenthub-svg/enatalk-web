...
export default function LandingPage() {
  ...
  function AuthBox() {
    ...
    return (
      <div style={{background:"linear-gradient(160deg,rgba(15,25,55,0.96),rgba(10,20,40,0.98))",backdropFilter:"blur(32px)",WebkitBackdropFilter:"blur(32px)",border:"1px solid rgba(255,255,255,0.10)",borderRadius:24,padding:"28px 24px",width:"100%",boxShadow:"0 40px 100px rgba(0,0,0,0.6)"}}>
        ...
        {showOtp?(
          ...
          <>
            ...
            {authMode==="signup"&&<p style={{fontSize:11,color:"rgba(255,255,255,0.3)",textAlign:"center",marginTop:14,lineHeight:1.65}}>No credit card · 14-day free trial · Cancel anytime</p>}
          </>
        )}
      </div>
    );
  ...
  function PolicyModal() {
    ...
    return(
      <div onClick={()=>setActivePolicy(null)} style={{position:"fixed",inset:0,zIndex:999,background:"rgba(2,8,16,0.92)",backdropFilter:"blur(12px)",display:"flex",alignItems:"flex-start",justifyContent:"center",padding:"40px 20px",overflowY:"auto"}}>
        <div onClick={e=>e.stopPropagation()} style={{width:"100%",maxWidth:720,background:"linear-gradient(160deg,rgba(15,25,55,0.98),rgba(10,20,40,0.99))",border:"1px solid rgba(255,255,255,0.10)",borderRadius:24,padding:"40px 40px 48px",boxShadow:"0 40px 100px rgba(0,0,0,0.7)",position:"relative"}}>
          <button onClick={()=>setActivePolicy(null)} style={{position:"absolute",top:20,right:20,background:"rgba(255,255,255,0.07)",border:"1px solid rgba(255,255,255,0.1)",color:"rgba(255,255,255,0.6)",width:36,height:36,borderRadius:"50%",cursor:"pointer",fontSize:18,display:"flex",alignItems:"center",justifyContent:"center",fontFamily:"'DM Sans',sans-serif"}}>×</button>
          <div style={{display:"flex",gap:8,marginBottom:28}}>
            {(["privacy","terms"] as const).map(p=>(
              <button key={p} onClick={()=>setActivePolicy(p)} style={{padding:"8px 20px",borderRadius:100,border:"none",cursor:"pointer",fontFamily:"'DM Sans',sans-serif",fontSize:13,fontWeight:700,background:activePolicy===p?"linear-gradient(135deg,#22C55E,#16A34A)":"rgba(255,255,255,0.06)",color:activePolicy===p?"#fff":"rgba(255,255,255,0.4)"}}>
    ...
    <div style={{fontFamily:"'DM Sans',sans-serif",background:"#040B1C",color:"#fff",minHeight:"100vh",overflowX:"hidden"}}>
      ...
      <style>{`
        *{box-sizing:border-box;margin:0;padding:0;}
        ...
        .sec{padding:80px 0;}
        /* ── GRID LAYOUTS ── */
        .hero-g{display:grid;grid-template-columns:1fr 420px;gap:64px;align-items:center;}
        ...
        .cta-g{display:grid;grid-template-columns:1fr 420px;gap:64px;align-items:center;}
        .comp-g{display:grid;grid-template-columns:1fr 1fr;gap:60px;align-items:start;}
        .price-g{display:grid;grid-template-columns:repeat(4,1fr);gap:16px;align-items:start;}
        /* ── NAV ── */
        .nav-links{display:flex;gap:32px;align-items:center;}
        .nav-btns{display:flex;gap:10px;}
        .hamburger{display:none;background:none;border:1px solid rgba(255,255,255,0.15);color:#fff;width:40px;height:40px;border-radius:10px;cursor:pointer;font-size:18px;align-items:center;justify-content:center;}
        .mob-menu{display:none;position:fixed;inset:0;top:64px;background:rgba(4,11,28,0.98);z-index:199;flex-direction:column;padding:24px;gap:4px;overflow-y:auto;}
        .mob-menu.open{display:flex;}
        .mob-link{padding:14px 16px;border-radius:12px;font-size:16px;font-weight:600;color:rgba(255,255,255,0.8);cursor:pointer;text-decoration:none;display:block;transition:background .2s;}
        .mob-link:hover{background:rgba(34,197,94,0.1);color:#22C55E;}
        /* ── RESPONSIVE ── */
        @media(max-width:1024px){
          .hero-g,.cta-g{grid-template-columns:1fr;gap:40px;}
          .price-g{grid-template-columns:repeat(2,1fr);}
          .comp-g{grid-template-columns:1fr;gap:32px;}
        }
        @media(max-width:768px){
          ...
          .case-g{grid-template-columns:1fr;}
        ...
        .faq-q{width:100%;background:none;border:none;color:#fff;font-family:'DM Sans',sans-serif;font-size:15px;font-weight:600;text-align:left;padding:20px 0;cursor:pointer;display:flex;justify-content:space-between;align-items:center;gap:16px;}
        .stat-n{font-family:'Bricolage Grotesque',sans-serif;font-weight:800;font-size:clamp(28px,4vw,42px);letter-spacing:-2px;background:linear-gradient(135deg,#22C55E,#34D399);-webkit-background-clip:text;-webkit-text-fill-color:transparent;}
        ...
        .comp-item:hover{border-color:rgba(34,197,94,0.25);}
        .foot-a{font-size:13px;color:rgba(255,255,255,0.38);font-weight:500;text-decoration:none;line-height:2.2;display:block;transition:color .2s;}
      ...
      <div aria-hidden style={{position:"fixed",inset:0,zIndex:0,pointerEvents:"none",overflow:"hidden"}}>
        <div style={{position:"absolute",inset:0,background:`radial-gradient(ellipse 80% 60% at 70% 0%,rgba(34,197,94,0.10) 0%,transparent 60%),radial-gradient(ellipse 60% 50% at 10% 80%,rgba(245,184,0,0.07) 0%,transparent 55%),#040B1C`}}/>
      ...
      <nav style={{position:"sticky",top:0,zIndex:200,background:scrolled?"rgba(4,11,28,0.96)":"rgba(4,11,28,0.88)",backdropFilter:"blur(20px)",borderBottom:"1px solid rgba(255,255,255,0.06)",transition:"all .35s"}}>
        <div className="w" style={{height:64,display:"flex",alignItems:"center",justifyContent:"space-between"}}>
          ...
          <a href="/" style={{textDecoration:"none",display:"flex",alignItems:"center",flexShrink:0}}>
            <img src="/enatalk-logo.webp" alt="EnaTalk" style={{height:100,width:"auto",objectFit:"contain",display:"block",filter:"drop-shadow(0 0 10px rgba(245,184,0,0.5)) drop-shadow(0 0 20px rgba(27,75,138,0.4)) brightness(1.15)"}} onError={e=>{(e.target as HTMLImageElement).style.display="none";}}/>
          </a>
      ...
      <div className={`mob-menu${menuOpen?" open":""}`}>
        {[["🚀 Features","#features"],["⚙️ How it works","#how-it-works"],["💰 Pricing","#pricing"],["✅ Compliance","#compliance"],["❓ FAQ","#faq"]].map(([l,h])=>(
          <a key={l} href={h} className="mob-link" onClick={()=>setMenuOpen(false)}>{l}</a>
        ))}
        <div style={{height:1,background:"rgba(255,255,255,0.08)",margin:"8px 0"}}/>
        <p style={{fontSize:11,color:"rgba(255,255,255,0.3)",fontWeight:700,letterSpacing:"0.1em",textTransform:"uppercase",padding:"8px 16px"}}>Legal</p>
        {[["🔒 Privacy Policy","/privacy-policy"],["📋 Terms of Service","/terms"],["💳 Refund Policy","/refund-policy"],["✅ Acceptable Use","/acceptable-use"],["📬 Contact Us","/contact"]].map(([l,h])=>(
          <a key={l} href={h} className="mob-link" onClick={()=>setMenuOpen(false)}>{l}</a>
        ))}
        <div style={{height:1,background:"rgba(255,255,255,0.08)",margin:"8px 0"}}/>
        <button onClick={()=>{setAuthMode("login");setMenuOpen(false);document.getElementById("auth-box")?.scrollIntoView({behavior:"smooth"});}} style={{width:"100%",padding:"14px",background:"rgba(255,255,255,0.06)",border:"1px solid rgba(255,255,255,0.12)",color:"#fff",borderRadius:12,fontWeight:600,fontSize:15,cursor:"pointer",fontFamily:"'DM Sans',sans-serif",marginTop:8}}>Log in</button>
        <button onClick={()=>{setAuthMode("signup");setMenuOpen(false);document.getElementById("auth-box")?.scrollIntoView({behavior:"smooth"});}} style={{width:"100%",padding:"14px",background:"linear-gradient(135deg,#22C55E,#16A34A)",border:"none",color:"#fff",borderRadius:12,fontWeight:700,fontSize:15,cursor:"pointer",fontFamily:"'DM Sans',sans-serif",boxShadow:"0 4px 14px rgba(34,197,94,0.35)",marginTop:8}}>Sign up free</button>
      </div>

      {/* ── TRUST STRIP ── */}
      <div style={{position:"relative",zIndex:1,background:"rgba(34,197,94,0.07)",borderBottom:"1px solid rgba(34,197,94,0.15)",padding:"10px 24px",textAlign:"center"}}>
        <p style={{fontSize:12,color:"rgba(255,255,255,0.55)",fontWeight:500,letterSpacing:".02em"}}>
          ...
          <span className="hide-mob"> &nbsp;·&nbsp; Compliant with WhatsApp Commerce & Business Policies &nbsp;·&nbsp; Data secured &nbsp;·&nbsp; 100% opt-in messaging</span>
        </p>
      </div>

      {/* ── HERO ── */}
      <section style={{position:"relative",zIndex:1,minHeight:"calc(100svh - 100px)",display:"flex",alignItems:"center",padding:"48px 0 64px"}}>
        <div className="w" style={{width:"100%"}}>
          <div className="hero-g">
            ...
            <div>
              ...
              <h1 className="fu fu2 nt" style={{fontSize:"clamp(34px,5vw,60px)",lineHeight:1.08,marginBottom:20}}>
                WhatsApp can grow<br/>your business by{" "}
                <span style={{background:"linear-gradient(135deg,#22C55E,#34D399)",WebkitBackgroundClip:"text",WebkitTextFillColor:"transparent"}}>3×</span>
                <br/>
                <span style={{background:"linear-gradient(135deg,#F5B800,#FBBF24)",WebkitBackgroundClip:"text",WebkitTextFillColor:"transparent"}}>Start free today</span>
              </h1>
              <p className="fu fu3" style={{fontSize:16,color:"rgba(255,255,255,0.58)",lineHeight:1.8,marginBottom:28,maxWidth:500,fontWeight:400}}>
              ...
              <div className="fu fu4" style={{display:"flex",flexWrap:"wrap",gap:10,marginBottom:36}}>
                {["✅ Free plan","⚡ 5-min setup","🔒 Official Meta API","🇮🇳 Made for India"].map(b=>(
                  <span key={b} className="tb">{b}</span>
                ))}
              </div>
              <div className="fu fu5" style={{display:"flex",gap:32,paddingTop:28,borderTop:"1px solid rgba(255,255,255,0.06)",flexWrap:"wrap"}}>
                {[["98%","Message open rate"],["3×","More repeat visits"],["5 min","Avg setup time"]].map(([n,l])=>(
                  <div key={l}><div className="stat-n">{n}</div><div style={{fontSize:12,color:"rgba(255,255,255,0.4)",marginTop:4,fontWeight:500}}>{l}</div></div>
                ))}
              </div>

              {/* Campaign preview — desktop only */}
              <div className="hide-mob" style={{marginTop:36,position:"relative",maxWidth:430,height:320}}>
                <div style={{position:"absolute",bottom:0,left:"50%",transform:"translateX(-50%)",width:200,height:280,zIndex:1}}>
                  <svg viewBox="0 0 220 300" fill="none" xmlns="http://www.w3.org/2000/svg" style={{width:"100%",height:"100%"}}>
                    ...
                    <rect x="25" y="80" width="26" height="42" rx="5" fill="#0d1117"/>
                    <rect x="25" y="80" width="26" height="42" rx="5" fill="url(#screenGrad)"/>
                ...
                <div style={{position:"absolute",top:0,right:0,width:240,background:"linear-gradient(160deg,rgba(13,22,48,0.97),rgba(8,16,36,0.98))",border:"1px solid rgba(34,197,94,0.22)",borderRadius:20,padding:"14px 16px",boxShadow:"0 24px 64px rgba(0,0,0,0.65)",backdropFilter:"blur(24px)",animation:"floatY 5s ease-in-out infinite",zIndex:10}}>
                  <div style={{display:"flex",alignItems:"center",gap:9,marginBottom:10}}>
                    ...
                    <div style={{flex:1}}><div style={{fontSize:12,fontWeight:700,color:"#fff"}}>Sharma Salon</div><div style={{fontSize:10,color:"rgba(255,255,255,0.4)"}}>Campaign · 248 contacts</div></div>
                ...
                <div style={{position:"absolute",bottom:50,left:0,background:"linear-gradient(135deg,rgba(13,22,48,0.96),rgba(8,16,36,0.97))",border:"1px solid rgba(245,184,0,0.22)",borderRadius:13,padding:"9px 13px",boxShadow:"0 12px 32px rgba(0,0,0,0.5)",backdropFilter:"blur(16px)",animation:"floatY 6.5s ease-in-out infinite",animationDelay:"1.8s",zIndex:10,display:"flex",alignItems:"center",gap:9}}>
            ...
            {/* RIGHT — Auth form (always visible) */}
            <div id="auth-box">
              <AuthBox/>
      ...
      <section style={{position:"relative",zIndex:1,background:"rgba(255,255,255,0.012)",padding:"48px 0"}}>
        <div className="w">
          <p style={{textAlign:"center",fontSize:11,color:"rgba(255,255,255,0.3)",marginBottom:20,fontWeight:600,letterSpacing:".12em",textTransform:"uppercase"}}>Works for every type of business</p>
      ...
      <section className="sec" style={{position:"relative",zIndex:1}}>
        <div className="w">
          <div style={{textAlign:"center",marginBottom:52}}>
            ...
            <h2 className="nt" style={{fontSize:"clamp(26px,3.8vw,46px)",marginTop:16,marginBottom:12}}>See how businesses <span style={{background:"linear-gradient(135deg,#22C55E,#34D399)",WebkitBackgroundClip:"text",WebkitTextFillColor:"transparent"}}>actually use EnaTalk</span></h2>
            <p style={{fontSize:15,color:"rgba(255,255,255,0.48)",maxWidth:460,margin:"0 auto",lineHeight:1.8}}>From salons to restaurants to jewellery shops — here's how real businesses grow with WhatsApp.</p>
          </div>
          <div className="case-g">
            {useCases.map((c,i)=>(
              <div key={i} className="case-card">
                ...
                <div style={{padding:"20px 20px 24px",background:"rgba(255,255,255,0.025)"}}>
                  <h3 style={{fontFamily:"'Bricolage Grotesque',sans-serif",fontSize:17,fontWeight:700,marginBottom:8,letterSpacing:"-.3px",lineHeight:1.3}}>{c.title}</h3>
      ...
      {/* ── TESTIMONIALS ── */}
      <section style={{position:"relative",zIndex:1,background:"rgba(255,255,255,0.012)",borderTop:"1px solid rgba(255,255,255,0.05)",borderBottom:"1px solid rgba(255,255,255,0.05)",padding:"72px 0"}}>
        <div className="w">
          <div style={{textAlign:"center",marginBottom:48}}>
            <span className="pill">What they say</span>
            <h2 className="nt" style={{fontSize:"clamp(26px,3.8vw,44px)",marginTop:16,marginBottom:12}}>Real business owners. <span style={{background:`linear-gradient(135deg,${GOLD},#FBBF24)`,WebkitBackgroundClip:"text",WebkitTextFillColor:"transparent"}}>Real results.</span></h2>
          </div>
          <div className="testi-g">
            {testimonials.map((t,i)=>(
              <div key={i} className="testi-card">
                <div style={{display:"flex",gap:4,marginBottom:14}}>{Array(t.rating).fill(0).map((_,j)=><span key={j} className="star">★</span>)}</div>
                <p style={{fontSize:14,color:"rgba(255,255,255,0.68)",lineHeight:1.8,marginBottom:18,fontStyle:"italic"}}>"{t.text}"</p>
      ...
      <section id="how-it-works" className="sec" style={{position:"relative",zIndex:1}}>
        <div className="w">
          <div style={{textAlign:"center",marginBottom:52}}>
            ...
            <h2 className="nt" style={{fontSize:"clamp(26px,3.8vw,46px)",marginTop:16,marginBottom:12}}>From zero to sending in <span style={{background:"linear-gradient(135deg,#22C55E,#34D399)",WebkitBackgroundClip:"text",WebkitTextFillColor:"transparent"}}>4 steps</span></h2>
            <p style={{fontSize:15,color:"rgba(255,255,255,0.48)",maxWidth:440,margin:"0 auto",lineHeight:1.8}}>No training needed. No IT team required. All from your phone.</p>
          </div>
          <div className="step-g">
            {steps.map((s,i)=>(
              <div key={i} className="sc">
                <div style={{fontFamily:"'Bricolage Grotesque',sans-serif",fontSize:"clamp(32px,5vw,52px)",fontWeight:800,color:"rgba(34,197,94,0.12)",marginBottom:14,letterSpacing:"-2px",lineHeight:1}}>{s.n}</div>
      ...
      <section id="features" className="sec" style={{position:"relative",zIndex:1,background:"rgba(255,255,255,0.012)",borderTop:"1px solid rgba(255,255,255,0.05)",borderBottom:"1px solid rgba(255,255,255,0.05)"}}>
        <div className="w">
          <div style={{textAlign:"center",marginBottom:52}}>
            ...
            <h2 className="nt" style={{fontSize:"clamp(26px,3.8vw,46px)",marginTop:16,marginBottom:12}}>Simple to use. <span style={{background:`linear-gradient(135deg,${GOLD},#FBBF24)`,WebkitBackgroundClip:"text",WebkitTextFillColor:"transparent"}}>Powerful results.</span></h2>
            <p style={{fontSize:15,color:"rgba(255,255,255,0.48)",maxWidth:460,margin:"0 auto",lineHeight:1.8}}>A clean, focused platform that gets out of your way and lets you connect with customers.</p>
          </div>
          <div className="feat-g">
            {features.map((f,i)=>(
              <div key={i} className="fc">
                <div style={{width:48,height:48,borderRadius:14,background:`${f.col}18`,border:`1px solid ${f.col}28`,display:"flex",alignItems:"center",justifyContent:"center",fontSize:22,marginBottom:18}}>{f.icon}</div>
      ...
      <section id="compliance" className="sec" style={{position:"relative",zIndex:1}}>
        <div className="w">
          <div className="comp-g">
            <div>
              <span className="pill" style={{marginBottom:16,display:"inline-flex"}}>Trust & Compliance</span>
              <h2 className="nt" style={{fontSize:"clamp(26px,3.8vw,44px)",marginTop:16,marginBottom:12}}>Built to meet <span style={{background:"linear-gradient(135deg,#22C55E,#34D399)",WebkitBackgroundClip:"text",WebkitTextFillColor:"transparent"}}>WhatsApp's Policy</span></h2>
              <p style={{fontSize:15,color:"rgba(255,255,255,0.48)",lineHeight:1.8,marginBottom:28,maxWidth:440}}>We take compliance seriously so your account stays protected and your business keeps growing.</p>
              <div style={{display:"flex",flexDirection:"column",gap:12}}>
                {complianceItems.map((c,i)=>(
                  <div key={i} className="comp-item">
                    <div style={{width:34,height:34,borderRadius:"50%",background:"rgba(34,197,94,0.1)",border:"1.5px solid rgba(34,197,94,0.3)",display:"flex",alignItems:"center",justifyContent:"center",color:G,fontSize:15,flexShrink:0,fontWeight:700}}>✓</div>
            ...
            <div style={{display:"flex",flexDirection:"column",gap:16}}>
              <div style={{background:"rgba(255,255,255,0.025)",border:"1px solid rgba(255,255,255,0.07)",borderRadius:20,padding:"24px 24px 20px"}}>
                <div style={{fontFamily:"'Bricolage Grotesque',sans-serif",fontWeight:700,fontSize:16,marginBottom:18,color:"#fff"}}>Supported business types</div>
                {["Retail & E-Commerce","Healthcare (non-prescription)","Education & Coaching","Financial Services","Real Estate","SaaS & Technology","Tours & Travel","Salons & Wellness"].map((t,i,arr)=>(
                  <div key={t} style={{display:"flex",justifyContent:"space-between",alignItems:"center",fontSize:13,padding:"11px 0",borderBottom:i<arr.length-1?"1px solid rgba(255,255,255,0.05)":"none"}}>
                    <span style={{color:"rgba(255,255,255,0.58)"}}>{t}</span>
                    <span style={{color:G,fontWeight:700,fontSize:12}}>✓ Supported</span>
                  </div>
                ))}
                <div style={{marginTop:14,padding:"12px 14px",background:"rgba(239,68,68,0.06)",border:"1px solid rgba(239,68,68,0.15)",borderRadius:12,fontSize:11,color:"rgba(255,255,255,0.38)",lineHeight:1.65}}>
              ...
              <div style={{background:"linear-gradient(135deg,rgba(34,197,94,0.08),rgba(34,197,94,0.04))",border:"1px solid rgba(34,197,94,0.2)",borderRadius:20,padding:"22px 24px",display:"flex",alignItems:"center",gap:16}}>
                <div style={{width:46,height:46,borderRadius:14,background:"rgba(34,197,94,0.15)",border:"1px solid rgba(34,197,94,0.3)",display:"flex",alignItems:"center",justifyContent:"center",fontSize:20,flexShrink:0}}>🔒</div>
                <div><div style={{fontWeight:700,fontSize:14,color:"#fff",marginBottom:4}}>Powered by Meta's Official API</div><div style={{fontSize:12,color:"rgba(255,255,255,0.45)",lineHeight:1.6}}>EnaTalk uses WhatsApp Business Cloud API — trusted by thousands of businesses globally.</div></div>
              </div>
            </div>
          </div>
        </div>
      ...
      <section className="sec" style={{position:"relative",zIndex:1,background:"rgba(255,255,255,0.012)",borderTop:"1px solid rgba(255,255,255,0.05)",borderBottom:"1px solid rgba(255,255,255,0.05)"}}>
        <div className="w">
          <div style={{textAlign:"center",marginBottom:52}}>
            ...
            <h2 className="nt" style={{fontSize:"clamp(26px,3.8vw,46px)",marginTop:16,marginBottom:12}}>How we compare</h2>
            <p style={{fontSize:15,color:"rgba(255,255,255,0.48)",maxWidth:440,margin:"0 auto",lineHeight:1.8}}>Other platforms charge more and deliver less. See the difference.</p>
          </div>
      ...
      <section id="pricing" className="sec" style={{position:"relative",zIndex:1}}>
        <div className="w">
          <div style={{textAlign:"center",marginBottom:48}}>
            ...
            <h2 className="nt" style={{fontSize:"clamp(26px,3.8vw,46px)",marginTop:16,marginBottom:12}}>Simple pricing. <span style={{background:`linear-gradient(135deg,${G},#34D399)`,WebkitBackgroundClip:"text",WebkitTextFillColor:"transparent"}}>No surprises.</span></h2>
            <p style={{fontSize:15,color:"rgba(255,255,255,0.48)",maxWidth:500,margin:"0 auto 24px",lineHeight:1.8}}>WhatsApp message costs passed through at <strong style={{color:"#fff"}}>zero markup</strong>. You pay exactly what Meta charges.</p>
          ...
          <div className="price-g" style={{marginBottom:36}}>
            {[
              {name:"Free",price:0,annualPrice:0,desc:"Try EnaTalk with no commitment",cta:"Start Free",ctaStyle:"ghost",popular:false,usage:[{label:"Messages / month",val:"500"},{label:"Contacts",val:"50"},{label:"WA numbers",val:"1"},{label:"Team members",val:"1"},{label:"Broadcasts",val:"—"},{label:"API calls / mo",val:"—"}],features:[{t:"Campaign builder (basic)",on:true},{t:"5 message templates",on:true},{t:"Manual contact add",on:true},{t:"Birthday automation",on:false},{t:"CSV import",on:false},{t:"Analytics",on:false},{t:"Webhooks & API",on:false},{t:"Priority support",on:false}]},
              {name:"Starter",price:299,annualPrice:239,desc:"For solo shop owners & freelancers",cta:"Start 14-day Trial",ctaStyle:"ghost",popular:false,usage:[{label:"Messages / month",val:"3,000"},{label:"Contacts",val:"500"},{label:"WA numbers",val:"1"},{label:"Team members",val:"2"},{label:"Broadcasts",val:"5 / mo"},{label:"API calls / mo",val:"10,000"}],features:[{t:"Campaign builder",on:true},{t:"20 message templates",on:true},{t:"CSV import (500 rows)",on:true},{t:"Birthday automation",on:true},{t:"Basic analytics",on:true},{t:"Hindi & regional",on:true},{t:"Webhooks & API",on:false},{t:"Priority support",on:false}]},
              {name:"Growth",price:799,annualPrice:639,desc:"For growing businesses & small chains",cta:"Start 14-day Trial",ctaStyle:"primary",popular:true,usage:[{label:"Messages / month",val:"10,000"},{label:"Contacts",val:"5,000"},{label:"WA numbers",val:"2"},{label:"Team members",val:"5"},{label:"Broadcasts",val:"Unlimited"},{label:"API calls / mo",val:"1,00,000"}],features:[{t:"Advanced campaigns",on:true},{t:"Unlimited templates",on:true},{t:"CSV import (unlimited)",on:true},{t:"Birthday & festival auto",on:true},{t:"Full analytics + reports",on:true},{t:"Hindi & regional",on:true},{t:"Webhooks & REST API",on:true},{t:"Email support (24hr)",on:true}]},
              {name:"Pro",price:1499,annualPrice:1199,desc:"For multi-location & enterprises",cta:"Start 14-day Trial",ctaStyle:"ghost",popular:false,usage:[{label:"Messages / month",val:"Unlimited"},{label:"Contacts",val:"Unlimited"},{label:"WA numbers",val:"5"},{label:"Team members",val:"Unlimited"},{label:"Broadcasts",val:"Unlimited"},{label:"API calls / mo",val:"Unlimited"}],features:[{t:"Everything in Growth",on:true},{t:"Advanced automation",on:true},{t:"Multi-number mgmt",on:true},{t:"Custom integrations",on:true},{t:"Analytics export",on:true},{t:"Role-based access",on:true},{t:"Dedicated account mgr",on:true},{t:"WhatsApp priority support",on:true}]},
            ].map(plan=>{
              const price=pricingAnnual?plan.annualPrice:plan.price;
              const isPrimary=plan.ctaStyle==="primary";
              return(
                <div key={plan.name} style={{background:plan.popular?"linear-gradient(160deg,rgba(34,197,94,0.09),rgba(34,197,94,0.03))":"rgba(255,255,255,0.025)",border:plan.popular?"1.5px solid rgba(34,197,94,0.35)":"1px solid rgba(255,255,255,0.08)",borderRadius:24,padding:"24px 20px",position:"relative",transform:plan.popular?"translateY(-8px)":"none",boxShadow:plan.popular?"0 24px 60px rgba(34,197,94,0.10)":"none"}}>
                  ...
                  <div style={{marginBottom:18}}>
                    ...
                    <div style={{display:"flex",alignItems:"flex-end",gap:4,marginBottom:4}}>
                      {price===0?<span style={{fontFamily:"'Bricolage Grotesque',sans-serif",fontWeight:800,fontSize:38,color:"#fff",letterSpacing:"-2px",lineHeight:1}}>Free</span>:<><span style={{fontFamily:"'Bricolage Grotesque',sans-serif",fontWeight:800,fontSize:38,color:"#fff",letterSpacing:"-2px",lineHeight:1}}>₹{price.toLocaleString("en-IN")}</span><span style={{fontSize:13,color:"rgba(255,255,255,0.38)",marginBottom:4}}>/mo</span></>}
                    </div>
                    <div style={{fontSize:12,color:"rgba(255,255,255,0.32)",marginBottom:6}}>{price===0?"No credit card required":pricingAnnual?"billed annually":"billed monthly"}</div>
                    <div style={{fontSize:12,color:"rgba(255,255,255,0.45)",lineHeight:1.6}}>{plan.desc}</div>
                  </div>
                  <a href="https://app.enatalk.com/login" style={{display:"block",textAlign:"center",padding:"11px",borderRadius:12,fontSize:13,fontWeight:700,textDecoration:"none",marginBottom:20,background:isPrimary?"linear-gradient(135deg,#22C55E,#16A34A)":"rgba(255,255,255,0.06)",border:isPrimary?"none":"1px solid rgba(255,255,255,0.12)",color:"#fff",boxShadow:isPrimary?"0 6px 20px rgba(34,197,94,0.35)":"none"}}>{plan.cta}</a>
                  <div style={{background:"rgba(255,255,255,0.03)",border:"1px solid rgba(255,255,255,0.06)",borderRadius:12,padding:"12px 14px",marginBottom:18}}>
                    <div style={{fontSize:10,fontWeight:700,color:"rgba(255,255,255,0.3)",textTransform:"uppercase",letterSpacing:"0.1em",marginBottom:10}}>Usage limits</div>
                    {plan.usage.map((u,i)=>(
                      <div key={i} style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"6px 0",borderBottom:i<plan.usage.length-1?"1px solid rgba(255,255,255,0.05)":"none"}}>
                        <span style={{fontSize:11,color:"rgba(255,255,255,0.42)"}}>{u.label}</span>
                        <span style={{fontSize:11,fontWeight:700,color:u.val==="—"?"rgba(255,255,255,0.2)":plan.popular?"#22C55E":"#fff"}}>{u.val}</span>
                      </div>
                    ))}
                  </div>
                  <div style={{fontSize:10,fontWeight:700,color:"rgba(255,255,255,0.3)",textTransform:"uppercase",letterSpacing:"0.1em",marginBottom:10}}>Features</div>
                  <ul style={{listStyle:"none",padding:0,margin:0,display:"flex",flexDirection:"column",gap:7}}>
                    {plan.features.map((f,i)=>(
                      <li key={i} style={{display:"flex",alignItems:"flex-start",gap:8,fontSize:12,color:f.on?(plan.popular?"rgba(255,255,255,0.85)":"rgba(255,255,255,0.65)"):"rgba(255,255,255,0.2)"}}>
                        <span style={{fontSize:12,flexShrink:0,marginTop:1,color:f.on?(plan.popular?"#22C55E":"rgba(34,197,94,0.7)"):"rgba(255,255,255,0.15)",fontWeight:700}}>{f.on?"✓":"✕"}</span>
          ...
          <div style={{background:"rgba(245,184,0,0.05)",border:"1px solid rgba(245,184,0,0.16)",borderRadius:16,padding:"18px 22px",display:"flex",gap:16,alignItems:"flex-start",maxWidth:800,margin:"0 auto 24px"}}>
            <span style={{fontSize:18,flexShrink:0}}>💡</span>
            <div>
              <div style={{fontWeight:700,fontSize:13,color:GOLD,marginBottom:8}}>WhatsApp message costs — billed separately at zero markup</div>
              <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(160px,1fr))",gap:10}}>
                {[{type:"Marketing",rate:"~₹0.58",desc:"Promotions & campaigns"},{type:"Utility",rate:"~₹0.14",desc:"Order updates, reminders"},{type:"Authentication",rate:"~₹0.12",desc:"OTPs & verifications"},{type:"Service",rate:"Free",desc:"Customer replies within 24hr"}].map(m=>(
                  <div key={m.type} style={{background:"rgba(255,255,255,0.03)",border:"1px solid rgba(255,255,255,0.06)",borderRadius:10,padding:"10px 12px"}}>
                    <div style={{fontSize:11,color:"rgba(255,255,255,0.4)",marginBottom:2}}>{m.type}</div>
                    <div style={{fontSize:15,fontWeight:700,color:"#fff",fontFamily:"'Bricolage Grotesque',sans-serif"}}>{m.rate}<span style={{fontSize:10,color:"rgba(255,255,255,0.35)",fontWeight:400}}> /conv</span></div>
          ...
          <div style={{textAlign:"center",padding:"14px",background:"rgba(255,255,255,0.02)",border:"1px solid rgba(255,255,255,0.06)",borderRadius:12}}>
      ...
      {/* ── FAQ ── */}
      <section id="faq" className="sec" style={{position:"relative",zIndex:1,background:"rgba(255,255,255,0.012)",borderTop:"1px solid rgba(255,255,255,0.05)"}}>
        <div className="w">
          <div style={{textAlign:"center",marginBottom:48}}>
            <span className="pill">FAQ</span>
            <h2 className="nt" style={{fontSize:"clamp(26px,3.8vw,46px)",marginTop:16,marginBottom:12}}>Common questions</h2>
          </div>
          <div style={{maxWidth:680,margin:"0 auto"}}>
            <div style={{background:"rgba(255,255,255,0.025)",border:"1px solid rgba(255,255,255,0.07)",borderRadius:24,padding:"4px 24px"}}>
              {faqs.map((faq,i)=>(
                <div key={i} className="faq-i" style={{borderBottom:"1px solid rgba(255,255,255,0.06)"}}>
                  <button className="faq-q" onClick={()=>setOpenFaq(openFaq===i?null:i)}>
                    <span style={{textAlign:"left"}}>{faq.q}</span>
                    <span style={{minWidth:28,height:28,borderRadius:"50%",background:openFaq===i?"linear-gradient(135deg,#22C55E,#16A34A)":"rgba(255,255,255,0.07)",color:"#fff",display:"inline-flex",alignItems:"center",justifyContent:"center",fontSize:18,fontWeight:700,transition:"all .25s",flexShrink:0,boxShadow:openFaq===i?"0 4px 12px rgba(34,197,94,0.35)":"none"}}>
                      {openFaq===i?"−":"+"}
                    </span>
                  </button>
      ...
      {/* ── FINAL CTA ── */}
      <section style={{position:"relative",zIndex:1,padding:"80px 0",borderTop:"1px solid rgba(255,255,255,0.05)",overflow:"hidden"}}>
        ...
        <div className="w" style={{position:"relative"}}>
          <div className="cta-g">
            <div>
              <span className="pill" style={{marginBottom:16,display:"inline-flex"}}>Get started today</span>
              <h2 className="nt" style={{fontSize:"clamp(26px,3.8vw,50px)",marginTop:16,marginBottom:16,lineHeight:1.08}}>Join 1,000+ businesses<br/>growing with <span style={{background:"linear-gradient(135deg,#22C55E,#34D399)",WebkitBackgroundClip:"text",WebkitTextFillColor:"transparent"}}>WhatsApp</span></h2>
              <p style={{fontSize:15,color:"rgba(255,255,255,0.48)",lineHeight:1.82,marginBottom:28,maxWidth:440}}>Free to start. No credit card. Works on your phone. Whether you run one shop or fifty locations — EnaTalk connects you to your customers in minutes.</p>
              <div style={{display:"flex",gap:10,flexWrap:"wrap"}}>
                {["🎂 Auto birthday & festivals","📱 100% mobile","🌐 Hindi & regional","💰 No hidden fees","🏢 Any business size"].map(b=>(
                  <span key={b} style={{fontSize:13,color:G,background:"rgba(34,197,94,0.08)",padding:"8px 16px",borderRadius:100,border:"1px solid rgba(34,197,94,0.18)",fontWeight:600}}>{b}</span>
                ))}
              </div>
            </div>
            <div><AuthBox/></div>
      ...
      <footer style={{background:"#020810",borderTop:"1px solid rgba(255,255,255,0.05)",padding:"40px 24px",position:"relative",zIndex:1}}>
        <div className="w" style={{display:"flex",alignItems:"flex-start",justifyContent:"space-between",flexWrap:"wrap",gap:32}}>
          <div style={{minWidth:200}}>
            <img src="/enatalk-logo.webp" alt="EnaTalk" style={{height:100,width:"auto",objectFit:"contain",maxWidth:160,display:"block",marginBottom:12,filter:"drop-shadow(0 0 10px rgba(245,184,0,0.5)) drop-shadow(0 0 20px rgba(27,75,138,0.4)) brightness(1.15)"}} onError={e=>{(e.target as HTMLImageElement).style.display="none";}}/>
          ...
          <div style={{display:"flex",gap:48,flexWrap:"wrap"}}>
            <div>
              <p style={{fontSize:11,color:"rgba(255,255,255,0.25)",fontWeight:700,letterSpacing:"0.1em",textTransform:"uppercase",marginBottom:10}}>Legal</p>
              {[["Privacy Policy","/privacy-policy"],["Terms of Service","/terms"],["Refund Policy","/refund-policy"],["Acceptable Use","/acceptable-use"],["Contact Us","/contact"]].map(([l,h])=>(
                <a key={l} href={h} className="foot-a">{l}</a>
              ))}
            </div>
            <div>
              <p style={{fontSize:11,color:"rgba(255,255,255,0.25)",fontWeight:700,letterSpacing:"0.1em",textTransform:"uppercase",marginBottom:10}}>Contact</p>
              <a href="mailto:support@enatalk.com" className="foot-a">support@enatalk.com</a>
              <a href="/contact" className="foot-a">Contact Page →</a>
              <a href="https://wa.me/919062211526" className="foot-a" target="_blank" rel="noopener noreferrer">WhatsApp Us →</a>
            </div>
          </div>
          <p style={{fontSize:11,color:"rgba(255,255,255,0.18)",width:"100%",textAlign:"center",paddingTop:20,borderTop:"1px solid rgba(255,255,255,0.04)"}}>
