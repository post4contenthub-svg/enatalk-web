...
export default function LandingPage() {
  ...
  const testimonials = [
    {
      ...
      name: "Priya Sharma", role: "Salon Owner, Delhi",
      text: "I send birthday offers to 300 customers every month automatically. My bookings went up 40% in the first 3 months.", rating: 5,
    },
    {
      img: "https://images.unsplash.com/photo-1566492031773-4f4e44671857?w=80&h=80&fit=crop&crop=face",
      name: "Ravi Patel", role: "Café Owner, Ahmedabad",
      text: "I used to spend 2 hours daily messaging customers. Now EnaTalk does it all. I just check the results.", rating: 5,
    },
    {
      img: "https://images.unsplash.com/photo-1573497019940-1c28c88b4f3e?w=80&h=80&fit=crop&crop=face",
      name: "Anita Reddy", role: "Jewellery Shop, Hyderabad",
      text: "Sent Diwali offers to 500 customers in one tap. Got 87 replies within 2 hours. Never saw this response before.", rating: 5,
    },
  ];

  const useCases = [
    {
      img: "https://images.unsplash.com/photo-1560066984-138dadb4c035?w=600&h=400&fit=crop",
      tag: "Salons & Spas", title: "Fill empty appointment slots instantly",
      desc: "Send last-minute offers to your contact list and watch bookings come in within minutes — all through WhatsApp.",
    },
    {
      img: "https://images.unsplash.com/photo-1554118811-1e0d58224f24?w=600&h=400&fit=crop",
      tag: "Cafés & Restaurants", title: "Daily specials that actually get read",
      desc: "WhatsApp messages have 98% open rate. Your daily menu special reaches every customer — not their spam folder.",
    },
    {
      img: "https://images.unsplash.com/photo-1573408301185-9519f94816b5?w=600&h=400&fit=crop",
      tag: "Retail & Jewellery", title: "Festival offers that drive footfall",
      desc: "Send personalised Diwali, Eid and Christmas offers with one tap. Customers feel valued — and they come back.",
    },
  ];
  ...
  const privacySections = [
    ...
    { h: "4. Data Sharing", b: "We do not sell or rent your personal data or your customers' contact data to any third party. We may share data with service providers (cloud hosting, payment processors) strictly to operate our platform, under confidentiality agreements." },
  ...
  function AuthBox() {
    ...
    return (
      <div style={{
        background: "linear-gradient(160deg,rgba(15,25,55,0.96) 0%,rgba(10,20,40,0.98) 100%)",
        backdropFilter: "blur(32px)", WebkitBackdropFilter: "blur(32px)",
        border: "1px solid rgba(255,255,255,0.10)", borderRadius: 24,
        padding: "30px 26px", width: "100%", maxWidth: 420,
        boxShadow: "0 40px 100px rgba(0,0,0,0.6), 0 0 0 1px rgba(34,197,94,0.08) inset",
      }}>
        <div style={{ display:"flex", background:"rgba(255,255,255,0.05)", borderRadius:13, padding:4, marginBottom:24 }}>
          {(["signup","login"] as const).map(m=>(
            <button key={m} onClick={()=>{setAuthMode(m);setError("");setShowOtp(false);}} style={{
              flex:1, padding:"10px 0", border:"none", borderRadius:10,
              cursor:"pointer", fontFamily:"'DM Sans',sans-serif",
              fontSize:14, fontWeight:700, transition:"all .2s",
              background: authMode===m ? "linear-gradient(135deg,#22C55E,#16A34A)" : "transparent",
              color: authMode===m ? "#fff" : "rgba(255,255,255,0.45)",
              boxShadow: authMode===m ? "0 4px 12px rgba(34,197,94,0.35)" : "none",
            }}>
        ...
        {showOtp ? (
          <form onSubmit={handleVerifyOtp} style={{display:"flex",flexDirection:"column",gap:12}}>
            ...
            <input type="text" inputMode="numeric" placeholder="Enter 6-digit code" value={otp}
              onChange={e=>setOtp(e.target.value)} required disabled={loading} style={inp}/>
          ...
          <>
            <button onClick={handleGoogleAuth} disabled={loading} style={{
              ...
              border:"1px solid rgba(255,255,255,0.10)",color:"#fff",borderRadius:12,
            ...
            <form onSubmit={handleEmailAuth} style={{display:"flex",flexDirection:"column",gap:10}}>
              <input type="email" placeholder="Email address" value={email}
                onChange={e=>setEmail(e.target.value)} required disabled={loading} style={inp}/>
              <input type="password" placeholder="Password" value={password}
                onChange={e=>setPassword(e.target.value)} required disabled={loading} style={inp}/>
              {authMode==="signup"&&(
                <select value={businessType} onChange={e=>setBusinessType(e.target.value)}
                  required disabled={loading}
                  style={{...inp,color:businessType?"#fff":"rgba(255,255,255,0.3)"}}>
  ...
  function PolicyModal() {
    ...
    return (
      <div onClick={()=>setActivePolicy(null)} style={{
        ...
        <div onClick={e=>e.stopPropagation()} style={{
          ...
          background:"linear-gradient(160deg,rgba(15,25,55,0.98),rgba(10,20,40,0.99))",
          border:"1px solid rgba(255,255,255,0.10)",borderRadius:24,
          ...
          <button onClick={()=>setActivePolicy(null)} style={{
            ...
            cursor:"pointer",fontSize:18,fontFamily:"'DM Sans',sans-serif",
          ...
          <div style={{display:"flex",gap:8,marginBottom:28}}>
            {(["privacy","terms"] as const).map(p=>(
              <button key={p} onClick={()=>setActivePolicy(p)} style={{
                padding:"8px 20px",borderRadius:100,border:"none",cursor:"pointer",
                fontFamily:"'DM Sans',sans-serif",fontSize:13,fontWeight:700,
                background: activePolicy===p ? "linear-gradient(135deg,#22C55E,#16A34A)" : "rgba(255,255,255,0.06)",
                color: activePolicy===p ? "#fff" : "rgba(255,255,255,0.4)",
                boxShadow: activePolicy===p ? "0 4px 14px rgba(34,197,94,0.3)" : "none",
              }}>
          ...
          {sections.map((sec,i)=>(
            <div key={i} style={{marginBottom:24}}>
              <h3 style={{fontFamily:"'Bricolage Grotesque',sans-serif",fontSize:16,fontWeight:700,color:"#22C55E",marginBottom:8,letterSpacing:"-0.2px"}}>{sec.h}</h3>
              {sec.b && <p style={{fontSize:13,color:"rgba(255,255,255,0.55)",lineHeight:1.85}}>{sec.b}</p>}
              {sec.list && (
                <ul style={{paddingLeft:18,marginTop:6}}>
                  {sec.list.map((item,j)=>(
                    <li key={j} style={{fontSize:13,color:"rgba(255,255,255,0.52)",lineHeight:1.8,marginBottom:4}}>{item}</li>
                  ))}
                </ul>
              )}
            </div>
    ...
    <div style={{fontFamily:"'DM Sans',sans-serif",background:"#040B1C",color:"#fff",minHeight:"100vh",overflowX:"hidden"}}>
      ...
      <style>{`
        *{box-sizing:border-box;margin:0;padding:0;}
        ...
        .policy-link:hover{color:#22C55E;}
      ...
      <nav style={{position:"sticky",top:0,zIndex:200,background:scrolled?"rgba(4,11,28,0.92)":"transparent",backdropFilter:scrolled?"blur(20px)":"none",borderBottom:scrolled?"1px solid rgba(255,255,255,0.06)":"none",transition:"all .35s"}}>
        <div className="w" style={{height:66,display:"flex",alignItems:"center",justifyContent:"space-between"}}>
          ...
          <img
            ...
            alt="EnaTalk"
            style={{height:100,width:"auto",objectFit:"contain",maxWidth:200,display:"block",mixBlendMode:"screen"}}
            onError={e=>{
          ...
          <div className="nav-mid" style={{display:"flex",gap:36,alignItems:"center"}}>
            ...
            <div style={{position:"relative"}} className="legal-dd">
              ...
              <div className="legal-dd-menu" style={{
                position:"absolute",top:"calc(100% + 12px)",left:"50%",transform:"translateX(-50%)",
                background:"rgba(8,16,36,0.98)",border:"1px solid rgba(255,255,255,0.10)",
                borderRadius:14,padding:"8px",minWidth:200,
                boxShadow:"0 20px 60px rgba(0,0,0,0.6)",backdropFilter:"blur(20px)",
                display:"none",flexDirection:"column",gap:2,zIndex:300,
              }}>
                {[["🔒 Privacy Policy","/privacy-policy"],["📋 Terms of Service","/terms"],["💳 Refund Policy","/refund-policy"],["✅ Acceptable Use","/acceptable-use"],["📬 Contact Us","/contact"]].map(([l,h])=>(
                  <a key={l} href={h} style={{fontSize:13,color:"rgba(255,255,255,0.65)",fontWeight:500,padding:"9px 14px",borderRadius:9,display:"block",transition:"background .15s"}
                  } onMouseOver={e=>(e.currentTarget.style.background="rgba(34,197,94,0.10)")}
                    onMouseOut={e=>(e.currentTarget.style.background="transparent")}>{l}</a>
                ))}
              </div>
            </div>
          </div>
          <div style={{display:"flex",gap:10}}>
            <button onClick={()=>{setAuthMode("login");document.getElementById("auth-box")?.scrollIntoView({behavior:"smooth"})}} style={{background:"transparent",border:"1px solid rgba(255,255,255,0.12)",color:"rgba(255,255,255,0.8)",padding:"9px 20px",borderRadius:10,fontWeight:600,fontSize:14,cursor:"pointer",fontFamily:"'DM Sans',sans-serif"}}>
              Log in
            </button>
            <button onClick={()=>{setAuthMode("signup");document.getElementById("auth-box")?.scrollIntoView({behavior:"smooth"})}} style={{background:"linear-gradient(135deg,#22C55E,#16A34A)",border:"none",color:"#fff",padding:"9px 20px",borderRadius:10,fontWeight:700,fontSize:14,cursor:"pointer",fontFamily:"'DM Sans',sans-serif",boxShadow:"0 4px 14px rgba(34,197,94,0.35)"}}>
              Sign up free
            </button>
          </div>
        </div>
      ...
      {/* ── TRUST STRIP ── NEW: visible compliance signal for Meta reviewers ── */}
      <div style={{position:"relative",zIndex:1,background:"rgba(34,197,94,0.07)",borderBottom:"1px solid rgba(34,197,94,0.15)",padding:"10px 28px",textAlign:"center"}}>
        <p style={{fontSize:12,color:"rgba(255,255,255,0.55)",fontWeight:500,letterSpacing:".02em"}}>
          ...
          &nbsp;·&nbsp; Compliant with WhatsApp Commerce & Business Policies
          &nbsp;·&nbsp; Data secured &nbsp;·&nbsp; 100% opt-in messaging
        </p>
      </div>
          ...
          <div className="hero-g">
            <div>
              <div className="fu fu1" style={{marginBottom:22}}>
                <span className="pill">
                  <span className="glow-dot" style={{width:7,height:7,borderRadius:"50%",background:G,display:"inline-block"}}/>
                  Official WhatsApp API &nbsp;·&nbsp; 100% Mobile-Friendly
                </span>
              </div>
              <h1 className="fu fu2 nt" style={{fontSize:"clamp(36px,4.8vw,60px)",lineHeight:1.08,marginBottom:22}}>
                WhatsApp can grow<br/>
                your business by{" "}
                <span style={{background:"linear-gradient(135deg,#22C55E 0%,#34D399 100%)",WebkitBackgroundClip:"text",WebkitTextFillColor:"transparent",display:"inline-block"}}>
                  3×
                </span>
                <br/>
                <span style={{background:"linear-gradient(135deg,#F5B800 0%,#FBBF24 100%)",WebkitBackgroundClip:"text",WebkitTextFillColor:"transparent"}}>
                  Start free today
                </span>
              </h1>
              <p className="fu fu3" style={{fontSize:17,color:"rgba(255,255,255,0.58)",lineHeight:1.82,marginBottom:34,maxWidth:500,fontWeight:400}}>
                Businesses using WhatsApp campaigns see up to <strong style={{color:"#fff"}}>3× more customer repeat visits</strong> compared to SMS or email.
                Send campaigns, birthday wishes & festival offers — from your phone, in minutes.
              </p>
              ...
              <div className="fu fu5" style={{display:"flex",gap:40,paddingTop:32,borderTop:"1px solid rgba(255,255,255,0.06)"}}>
                {[["98%","Message open rate"],["3×","More repeat visits"],["5 min","Average setup time"]].map(([n,l])=>(
                  <div key={l}>
                    ...
                    <div style={{fontSize:12,color:"rgba(255,255,255,0.4)",marginTop:5,fontWeight:500}}>{l}</div>
                  </div>
                ))}
              </div>
              {/* ── HERO VISUAL: human + phone + chat overlay ── */}
              <div className="hide-mob" style={{marginTop:40,position:"relative",maxWidth:430,height:340}}>

                {/* Human silhouette holding phone */}
                <div style={{position:"absolute",bottom:0,left:"50%",transform:"translateX(-50%)",width:220,height:300,zIndex:1}}>
                  <svg viewBox="0 0 220 300" fill="none" xmlns="http://www.w3.org/2000/svg" style={{width:"100%",height:"100%"}}>
                    <defs>
                      <linearGradient id="bodyGrad" x1="72" y1="130" x2="148" y2="240" gradientUnits="userSpaceOnUse">
                        <stop offset="0%" stopColor="#1e3a2f"/><stop offset="100%" stopColor="#0f2018"/>
                      </linearGradient>
                      <linearGradient id="skinGrad" x1="0" y1="0" x2="1" y2="1">
                        <stop offset="0%" stopColor="#c68642"/><stop offset="100%" stopColor="#a0522d"/>
                      </linearGradient>
                      <linearGradient id="legGrad" x1="0" y1="0" x2="0" y2="1">
                        <stop offset="0%" stopColor="#162d22"/><stop offset="100%" stopColor="#0a1a13"/>
                      </linearGradient>
                      <linearGradient id="screenGrad" x1="0" y1="0" x2="0" y2="1">
                        <stop offset="0%" stopColor="rgba(34,197,94,0.35)"/><stop offset="100%" stopColor="rgba(34,197,94,0.05)"/>
                      </linearGradient>
                      <radialGradient id="glowGrad" cx="50%" cy="50%" r="50%">
                        <stop offset="0%" stopColor="rgba(34,197,94,0.25)"/><stop offset="100%" stopColor="rgba(34,197,94,0)"/>
                      </radialGradient>
                    </defs>
                    {/* ground glow */}
                    <ellipse cx="110" cy="292" rx="72" ry="10" fill="rgba(34,197,94,0.12)"/>
                    {/* legs */}
                    <rect x="82" y="232" width="26" height="62" rx="13" fill="url(#legGrad)"/>
                    <rect x="112" y="232" width="26" height="62" rx="13" fill="url(#legGrad)"/>
                    {/* body */}
                    ...
                    {/* hair */}
                    <ellipse cx="110" cy="79" rx="33" ry="19" fill="#111827"/>
                    <ellipse cx="110" cy="75" rx="29" ry="15" fill="#0d1117"/>
                    {/* face */}
                    <ellipse cx="100" cy="107" rx="4" ry="5" fill="#6b3a1f" opacity="0.6"/>
                    <ellipse cx="120" cy="107" rx="4" ry="5" fill="#6b3a1f" opacity="0.6"/>
                ...
                <div style={{
                  position:"absolute",top:0,right:0,width:252,
                  background:"linear-gradient(160deg,rgba(13,22,48,0.97),rgba(8,16,36,0.98))",
                  border:"1px solid rgba(34,197,94,0.22)",borderRadius:20,
                  padding:"15px 16px",
                  boxShadow:"0 24px 64px rgba(0,0,0,0.65), 0 0 0 1px rgba(34,197,94,0.07) inset",
                  backdropFilter:"blur(24px)",
                  animation:"floatY 5s ease-in-out infinite",zIndex:10,
                }}>
                  <div style={{display:"flex",alignItems:"center",gap:9,marginBottom:11}}>
                    <div style={{width:34,height:34,borderRadius:"50%",background:"linear-gradient(135deg,#22C55E,#16A34A)",display:"flex",alignItems:"center",justifyContent:"center",fontSize:15,flexShrink:0,boxShadow:"0 4px 12px rgba(34,197,94,0.4)"}}>💬</div>
                    <div style={{flex:1,minWidth:0}}>
                      <div style={{fontSize:12,fontWeight:700,color:"#fff"}}>Sharma Salon</div>
                      <div style={{fontSize:10,color:"rgba(255,255,255,0.4)",marginTop:1}}>Campaign · 248 contacts</div>
                    </div>
                    <div style={{fontSize:10,color:G,fontWeight:700,whiteSpace:"nowrap"}}>✓✓ 231 read</div>
                  </div>
                  <div style={{background:"rgba(34,197,94,0.06)",border:"1px solid rgba(34,197,94,0.14)",borderLeft:`3px solid ${G}`,borderRadius:"4px 13px 13px 13px",padding:"9px 11px",fontSize:11.5,lineHeight:1.7,marginBottom:10}}>
                    🎂 <strong style={{color:"#fff"}}>Happy Birthday Priya ji!</strong><br/>
                    Get <strong style={{color:G}}>20% off</strong> at Sharma Salon today! 🎉
                  </div>
                ...
                <div style={{
                  ...
                  background:"linear-gradient(135deg,rgba(13,22,48,0.96),rgba(8,16,36,0.97))",
                  border:"1px solid rgba(245,184,0,0.22)",borderRadius:13,
                  padding:"9px 13px",
                  boxShadow:"0 12px 32px rgba(0,0,0,0.5)",
                  backdropFilter:"blur(16px)",
                  animation:"floatY 6.5s ease-in-out infinite",animationDelay:"1.8s",
                  zIndex:10,display:"flex",alignItems:"center",gap:9,
                }}>
                  <div style={{width:26,height:26,borderRadius:"50%",background:"rgba(245,184,0,0.12)",border:"1px solid rgba(245,184,0,0.28)",display:"flex",alignItems:"center",justifyContent:"center",fontSize:12,flexShrink:0}}>⚡</div>
                  <div>
                    <div style={{fontSize:11,fontWeight:700,color:"#fff",lineHeight:1.2}}>Campaign sent!</div>
                    <div style={{fontSize:9.5,color:"rgba(255,255,255,0.4)",marginTop:1}}>500 messages · 2 min ago</div>
                  </div>
                </div>
      ...
      <section style={{position:"relative",zIndex:1,background:"rgba(255,255,255,0.012)",padding:"52px 0"}}>
        <div className="w">
          ...
          <div className="biz-g">
            {businesses.map(b=>(<div key={b} className="bc">{b}</div>))}
          </div>
        </div>
      </section>
      <div className="div-glow"/>
        ...
        <div className="w">
          <div style={{textAlign:"center",marginBottom:60}}>
            ...
            <h2 className="nt" style={{fontSize:"clamp(28px,3.8vw,46px)",marginTop:18,marginBottom:14}}>
              See how businesses{" "}
              <span style={{background:"linear-gradient(135deg,#22C55E,#34D399)",WebkitBackgroundClip:"text",WebkitTextFillColor:"transparent"}}>actually use EnaTalk</span>
            </h2>
            <p style={{fontSize:16,color:"rgba(255,255,255,0.48)",maxWidth:460,margin:"0 auto",lineHeight:1.8}}>
              From salons to restaurants to jewellery shops — here's how real businesses grow with WhatsApp.
            </p>
          </div>
            ...
            {useCases.map((c,i)=>(
              <div key={i} className="case-card">
                <div style={{height:200,overflow:"hidden",position:"relative"}}>
                  <img src={c.img} alt={c.tag} style={{width:"100%",height:"100%",objectFit:"cover",display:"block",transition:"transform .4s"}}
      ...
      <section style={{position:"relative",zIndex:1,background:"rgba(255,255,255,0.012)",borderTop:"1px solid rgba(255,255,255,0.05)",borderBottom:"1px solid rgba(255,255,255,0.05)",padding:"80px 0"}}>
        <div className="w">
          <div style={{textAlign:"center",marginBottom:52}}>
            ...
            <h2 className="nt" style={{fontSize:"clamp(28px,3.8vw,44px)",marginTop:18,marginBottom:14}}>
              Real business owners.{" "}
              <span style={{background:`linear-gradient(135deg,${GOLD},#FBBF24)`,WebkitBackgroundClip:"text",WebkitTextFillColor:"transparent"}}>Real results.</span>
            </h2>
          </div>
              ...
              <div key={i} className="testi-card">
                ...
                <div style={{display:"flex",alignItems:"center",gap:12}}>
                  ...
                  <div>
                    ...
                    <div style={{fontSize:12,color:"rgba(255,255,255,0.42)",marginTop:2}}>{t.role}</div>
                  </div>
                </div>
              </div>
            ))}
        ...
        <div className="w">
          <div style={{textAlign:"center",marginBottom:60}}>
            ...
            <h2 className="nt" style={{fontSize:"clamp(28px,3.8vw,46px)",marginTop:18,marginBottom:14}}>
              ...
              <span style={{background:"linear-gradient(135deg,#22C55E,#34D399)",WebkitBackgroundClip:"text",WebkitTextFillColor:"transparent"}}>4 steps</span>
            </h2>
          ...
          <div className="step-g">
            {steps.map((s,i)=>(
              <div key={i} className="sc">
                ...
                <div style={{fontWeight:700,fontSize:16,marginBottom:10,letterSpacing:"-.2px"}}>{s.title}</div>
      ...
      <section id="features" className="sec" style={{position:"relative",zIndex:1,background:"rgba(255,255,255,0.012)",borderTop:"1px solid rgba(255,255,255,0.05)",borderBottom:"1px solid rgba(255,255,255,0.05)"}}>
        <div className="w">
          <div style={{textAlign:"center",marginBottom:60}}>
            ...
            <h2 className="nt" style={{fontSize:"clamp(28px,3.8vw,46px)",marginTop:18,marginBottom:14}}>
              Simple to use.{" "}
              <span style={{background:`linear-gradient(135deg,${GOLD},#FBBF24)`,WebkitBackgroundClip:"text",WebkitTextFillColor:"transparent"}}>Powerful results.</span>
            </h2>
            <p style={{fontSize:16,color:"rgba(255,255,255,0.48)",maxWidth:460,margin:"0 auto",lineHeight:1.8}}>
              A clean, focused platform that gets out of your way and lets you connect with customers.
            </p>
          </div>
          <div className="feat-g">
            {features.map((f,i)=>(
              <div key={i} className="fc">
                ...
                <div style={{fontWeight:700,fontSize:16,marginBottom:10,letterSpacing:"-.2px"}}>{f.title}</div>
        ...
        <div className="w">
          <div className="comp-g">
            ...
            <div>
              ...
              <h2 className="nt" style={{fontSize:"clamp(28px,3.8vw,44px)",marginTop:18,marginBottom:14}}>
                Built to meet{" "}
                <span style={{background:"linear-gradient(135deg,#22C55E,#34D399)",WebkitBackgroundClip:"text",WebkitTextFillColor:"transparent"}}>WhatsApp's Policy</span>
              </h2>
              <p style={{fontSize:16,color:"rgba(255,255,255,0.48)",lineHeight:1.8,marginBottom:36,maxWidth:440}}>
                We take compliance seriously so your account stays protected and your business keeps growing — without interruptions.
              </p>
              <div style={{display:"flex",flexDirection:"column",gap:12}}>
                {complianceItems.map((c,i)=>(
                  <div key={i} className="comp-item">
                    ...
                    <div>
                      <div style={{fontWeight:700,fontSize:14,marginBottom:4,color:"#fff"}}>{c.title}</div>
                      <div style={{fontSize:13,color:"rgba(255,255,255,0.48)",lineHeight:1.7}}>{c.desc}</div>
                    </div>
                  </div>
                ))}
              </div>
            ...
            {/* RIGHT — supported business types card */}
            <div style={{display:"flex",flexDirection:"column",gap:16}}>
              ...
              <div style={{background:"linear-gradient(135deg,rgba(34,197,94,0.08),rgba(34,197,94,0.04))",border:"1px solid rgba(34,197,94,0.2)",borderRadius:20,padding:"24px 28px",display:"flex",alignItems:"center",gap:16}}>
                ...
                <div>
                  <div style={{fontWeight:700,fontSize:14,color:"#fff",marginBottom:4}}>Powered by Meta's Official API</div>
                  <div style={{fontSize:12,color:"rgba(255,255,255,0.45)",lineHeight:1.6}}>EnaTalk uses WhatsApp Business Cloud API — the same infrastructure trusted by thousands of businesses globally.</div>
                </div>
              </div>
            </div>
          </div>
      ...
      <section className="sec" style={{position:"relative",zIndex:1,background:"rgba(255,255,255,0.012)",borderTop:"1px solid rgba(255,255,255,0.05)",borderBottom:"1px solid rgba(255,255,255,0.05)"}}>
        <div className="w">
          <div style={{textAlign:"center",marginBottom:60}}>
            ...
            <p style={{fontSize:16,color:"rgba(255,255,255,0.48)",maxWidth:460,margin:"0 auto",lineHeight:1.8}}>Other platforms charge more and deliver less. See the difference.</p>
          </div>
            ...
            <table className="tbl">
              <thead>
                <tr>
                  ...
                  <th style={{textAlign:"center",background:"linear-gradient(135deg,#16A34A,#22C55E)",color:"#fff",fontWeight:800,fontSize:15,letterSpacing:"-.2px"}}>EnaTalk ✦</th>
        ...
        <div className="w" style={{position:"relative"}}>
          <div className="cta-g">
            <div>
              ...
              <h2 className="nt" style={{fontSize:"clamp(28px,3.8vw,50px)",marginTop:18,marginBottom:18,lineHeight:1.08}}>
                Join 1,000+ businesses<br/>
                growing with{" "}
                <span style={{background:"linear-gradient(135deg,#22C55E,#34D399)",WebkitBackgroundClip:"text",WebkitTextFillColor:"transparent"}}>WhatsApp</span>
              </h2>
              <p style={{fontSize:16,color:"rgba(255,255,255,0.48)",lineHeight:1.82,marginBottom:32,maxWidth:440}}>
                Free to start. No credit card. Works on your phone. Whether you run one shop or fifty locations — EnaTalk connects you to your customers in minutes.
              </p>
      ...
      <footer style={{background:"#020810",borderTop:"1px solid rgba(255,255,255,0.05)",padding:"36px 28px",position:"relative",zIndex:1}}>
        <div className="w" style={{display:"flex",alignItems:"flex-start",justifyContent:"space-between",flexWrap:"wrap",gap:32}}>
          <div>
            <img src="/enatalk-logo.webp" alt="EnaTalk" style={{height:100,width:"auto",objectFit:"contain",maxWidth:180,display:"block",marginBottom:10,mixBlendMode:"screen"}} onError={e=>{(e.target as HTMLImageElement).style.display="none"}}/>
          ...
          <div style={{display:"flex",gap:0,flexWrap:"wrap",alignItems:"flex-start"}}>
            ...
            <div style={{display:"flex",flexDirection:"column",gap:4,minWidth:160}}>
              ...
              {[["Privacy Policy","/privacy-policy"],["Terms of Service","/terms"],["Refund Policy","/refund-policy"],["Acceptable Use","/acceptable-use"],["Contact Us","/contact"]].map(([l,h])=>(
                <a key={l} href={h} style={{fontSize:13,color:"rgba(255,255,255,0.38)",fontWeight:500,textDecoration:"none",transition:"color .2s",lineHeight:2}}
                  onMouseOver={e=>(e.currentTarget.style.color="#22C55E")}
                  onMouseOut={e=>(e.currentTarget.style.color="rgba(255,255,255,0.38)")}>{l}</a>
              ))}
            </div>
            {/* Contact column */}
            <div style={{display:"flex",flexDirection:"column",gap:4,minWidth:160,marginLeft:48}}>
              ...
              <a href="mailto:support@enatalk.com" style={{fontSize:13,color:"rgba(255,255,255,0.38)",fontWeight:500,textDecoration:"none",lineHeight:2}}
                onMouseOver={e=>(e.currentTarget.style.color="#22C55E")}
                onMouseOut={e=>(e.currentTarget.style.color="rgba(255,255,255,0.38)")}>support@enatalk.com</a>
              <a href="/contact" style={{fontSize:13,color:"rgba(255,255,255,0.38)",fontWeight:500,textDecoration:"none",lineHeight:2}}
                onMouseOver={e=>(e.currentTarget.style.color="#22C55E")}
                onMouseOut={e=>(e.currentTarget.style.color="rgba(255,255,255,0.38)")}>Contact Page →</a>
            </div>
          </div>
