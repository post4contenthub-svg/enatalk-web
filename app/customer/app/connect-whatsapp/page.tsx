...
export default function ConnectWhatsAppPage() {
  ...
  useEffect(() => {
    if (window.FB) {
      window.FB.init({
        ...
        cookie: true, xfbml: true, version: "v22.0",
      });
      setSdkReady(true);
      return;
    }
  ...
  // ── Handle code from URL (after redirect back) ──
  useEffect(() => {
    const params = new URLSearchParams(window.location.search);
    const code = params.get("code");
    if (code) {
      handleMetaResponse(code);
    }
  }, []);
  ...
  // ── Connect via popup (with fallback to redirect) ──
  function handleConnect() {
    ...
    try {
      ...
      handleRedirectConnect();
    }
  }

  // ── Redirect fallback (no popup needed) ──
  ...
  if (status === "loading") {
    return (
      <div style={{ display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", minHeight: "60vh", gap: 20, fontFamily: "'DM Sans',sans-serif" }}>
        ...
        <style>{`@keyframes spin{to{transform:rotate(360deg)}}`}</style>
        <p style={{ color: "#fff", fontSize: 16, fontWeight: 700 }}>Connecting to Meta…</p>
        <p style={{ color: MUTED, fontSize: 13, textAlign: "center", maxWidth: 280 }}>
          If nothing happens in 3 seconds, click below to connect via redirect instead.
        </p>
  ...
  const steps = [
    { n: "1", title: "Tap the button below", desc: "A secure Meta page opens — like 'Login with Google'." },
    { n: "2", title: "Log in to Facebook", desc: "Use your existing Facebook account or create one." },
    { n: "3", title: "Enter your phone number", desc: "This becomes your WhatsApp Business number." },
    { n: "4", title: "Enter the OTP", desc: "Meta sends a code to your phone. Enter it to verify." },
  ...
  return (
    <div style={{ maxWidth: 540, margin: "0 auto", fontFamily: "'DM Sans',sans-serif" }}>
        ...
        <div style={{ display: "flex", gap: 10, alignItems: "flex-start" }}>
          ...
          <div>
            ...
            <p style={{ fontSize: 13, color: "rgba(255,255,255,0.58)", lineHeight: 1.65 }}>
              If you connect a number already active on WhatsApp, it will be <strong style={{ color: "#fff" }}>removed from that device</strong> and chat history will be deleted.
            </p>
          </div>
        </div>
      </div>
      ...
      <div className="fu fu4">
        ...
        {/* Secondary — direct redirect (no popup) */}
